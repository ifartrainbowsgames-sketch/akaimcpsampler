...
export const useStore = create<UIState>((set, get) => ({
  ...
  async resampleToPad(padIndex) {
    const p = get().project;
    const { kitVolume, compressor, knobFX, knobFXParams, knobFXBypass } = get();
    const blob = await renderToWav({
      ...
      compressor,
      knobFX: knobFXBypass ? undefined : { id: knobFX, params: knobFXParams, bypass: knobFXBypass },
    });
    if (!blob) return;
  ...
  async exportSequence() {
    const p = get().project;
    const { kitVolume, compressor, knobFX, knobFXParams, knobFXBypass, bank, seqSlot } = get();
    const blob = await renderToWav({
      project: p,
